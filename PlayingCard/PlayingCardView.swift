//
//  PlayingCardViev.swift
//  PlayingCard
//
//  Created by муса магсумов on 18/02/2019.
   ...
   var isFaceUp:Bool = true{didSet{setNeedsDisplay();setNeedsLayout()}}
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp{
            ...
            if let isNotFaceUpCard = UIImage(named: "ASOC" ,in:Bundle(for: self.classForCoder),compatibleWith: traitCollection){
                bounds.origin = bounds.origin.offsetByMinus(dx: cornerOffset, dy: cornerOffset)
                isNotFaceUpCard.draw(in: bounds.sized(to: CGSize(width: bounds.width + 10, height: bounds.height + 15 )))
            }
        }
    }
}
...
extension PlayingCardView{
    private struct SizeRatio {
        ...
        static let cornerRadiusBoundsHeight:CGFloat = 0.06
        static let сornerOffsetToCorenerRadius:CGFloat = 0.33
    ...
    private var cornerOffset:CGFloat{
        return cornerRadius * SizeRatio.сornerOffsetToCorenerRadius
    }
...
extension CGRect{
    ...
    func zoom(by scale:CGFloat)->CGRect{
        let newWidht = width * scale
        let newHight = height * scale
        return insetBy(dx: (width - newWidht)/2, dy: (height - newHight)/2)
    }
...
extension CGPoint{
    ...
    func  offsetByMinus(dx:CGFloat,dy:CGFloat)->CGPoint{
        return CGPoint(x: x-dx, y: y-dy)
    }
